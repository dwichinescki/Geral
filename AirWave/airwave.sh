...
urlbase="https://airwave.paranabanco.jm"
...
if [ $# -eq 0 ] ; then
    ...
    aps=`curl -s -k --header "$TOKEN" -b /usr/lib/zabbix/externalscripts/cookie $url | xmllint --xpath "//ap[model!='Instant Virtual Controller']" --format - | grep 'name' | grep -o 'PRBAP[A-Z]\+[0-9]\+'`
    count=`echo "${aps}" | wc -l`
